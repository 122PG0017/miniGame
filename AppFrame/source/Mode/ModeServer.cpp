...
namespace AppFrame {
	...
	ModeServer::ModeServer()
	{
		...
		_uid_count = 1;
		_nowMode.clear();
		_skipUpdateMode.clear();
		_skipRenderMode.clear();
		_pauseUpdateMode.clear();
		_slowRate = 1.0f;
		_slowStart = 0;
		_slowDuration = 0.0f;
	}
	...
	int ModeServer::Process(InputManager& input)
	{
		...
		for (int i = 0; i < _nowMode.size(); i++)
		{
			if (_vMode.find(_nowMode[i].c_str()) != _vMode.end()) {
				...
				// Processコール
				if (!_vMode.at(_nowMode[i].c_str())->Process(input))
				{
	...
	int ModeServer::SkipUpdateUnderLayer() {
		this->_skipUpdateMode = this->_nowMode;
		return 0;
	}

	// 今処理しているレイヤーより下のレイヤーは、描画を呼ばない
	int ModeServer::SkipRenderUnderLayer() {
		this->_skipRenderMode = this->_nowMode;
		return 0;
	}
	...
	int ModeServer::PauseProcessUnderLayer() {
		this->_pauseUpdateMode = this->_nowMode;
		return 0;
	}
